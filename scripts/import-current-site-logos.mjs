...
const targets = [
  {
    id: 'supun-traders',
    companyIds: ['supun-traders'],
    aliases: ['supun traders', 'supun traders distributors'],
  },
  {
    id: 'supun-super-center',
    companyIds: ['supun-super-center', 'supun-super-centre'],
    aliases: ['supun super center', 'supun super centre'],
  },
  {
    id: 'supun-arcade-residency',
    companyIds: ['supun-arcade-residency', 'supun-arcade'],
    aliases: ['supun arcade residency', 'supun arcade'],
  },
  { id: 'area-56', companyIds: ['area-56', 'area56'], aliases: ['area 56'] },
  {
    id: 'supun-aerosoft',
    companyIds: ['supun-aerosoft', 'aerosoft'],
    aliases: ['supun aerosoft', 'ymac smart', 'aerosoft'],
  },
  {
    id: 'aerostar-home-appliances',
    companyIds: ['aerostar-home-appliances', 'aero-star', 'aerostar'],
    aliases: ['aerostar home appliances', 'aero star', 'aerostar'],
  },
  ...
  {
    id: 'fuji-industries',
    companyIds: ['fuji-industries', 'fuji'],
    aliases: ['fuji industries', 'fuji'],
  },
...
const normalize = (value = '') =>
  value
    .toLowerCase()
    .replace(/\(pvt\)|private|limited|ltd|pvt|[^a-z0-9]+/g, ' ')
    .trim()
    .replace(/\s+/g, ' ');

const fetchJson = async (url) => {
  const response = await fetch(url, {
    headers: { Accept: 'application/json', 'User-Agent': 'SupunGroupWebsiteV2LogoImporter/1.0' },
  });
...
const absoluteUrl = (value) => {
  ...
  try {
    return new URL(value, SOURCE_SITE).toString();
  } catch {
    return null;
  }
};
...
const findCompany = (companies, target) => {
  const exact = companies.find((company) =>
    target.companyIds.includes(String(company.id || '').toLowerCase()),
  );
  if (exact) return exact;
  const aliases = target.aliases.map(normalize);
  return companies.find((company) => {
    const values = [company.name, company.shortName, company.id].map((value) =>
...
const extensionFor = (contentType, url) => {
  ...
  try {
    ...
    if (['svg', 'webp', 'jpg', 'jpeg', 'png', 'gif'].includes(ext))
...
const downloadLogo = async (target, sourceUrl) => {
  const response = await fetch(sourceUrl, {
    headers: { 'User-Agent': 'SupunGroupWebsiteV2LogoImporter/1.0' },
    redirect: 'follow',
  });
  if (!response.ok) throw new Error(`${sourceUrl} returned ${response.status}`);
  const contentType = response.headers.get('content-type') || '';
  if (!contentType.startsWith('image/'))
    throw new Error(`${sourceUrl} is not an image (${contentType || 'unknown content type'})`);
...
let brands = [];
try {
  companies = await fetchJson(`${SOURCE_SITE}/api/companies`);
} catch (error) {
  console.warn(`Companies API: ${error.message}`);
}
try {
  brands = await fetchJson(`${SOURCE_SITE}/api/brands`);
} catch (error) {
  console.warn(`Brands API: ${error.message}`);
}
