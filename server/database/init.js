...
export function initDatabase() {
  ...
  try {
    ...
    const hasGoogleMapsLink = tableInfo.some((col) => col.name === 'googleMapsLink');
  ...
  try {
    ...
    const hasShortDescription = productsInfo.some((col) => col.name === 'shortDescription');
    const hasLongDescription = productsInfo.some((col) => col.name === 'longDescription');
    const hasImageUrl = productsInfo.some((col) => col.name === 'imageUrl');
    const hasWholesalePrice = productsInfo.some((col) => col.name === 'wholesalePrice');
  ...
  try {
    const productVariationsInfo = db.prepare('PRAGMA table_info(product_variations)').all();
    const hasVariationImageUrl = productVariationsInfo.some((col) => col.name === 'imageUrl');

    if (!hasVariationImageUrl) {
      console.log('📦 Running migration: Adding imageUrl to product_variations...');
      db.exec(`ALTER TABLE product_variations ADD COLUMN imageUrl TEXT`);
      console.log('✅ Migration completed: product_variations.imageUrl column added');
    }
  } catch (error) {
