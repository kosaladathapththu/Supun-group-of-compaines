...
const categoryDefinitions = [
  { name: "Safety Equipment", slug: "safety-equipment", description: "Protective products including SLS-certified motorcycle helmets." },
  { name: "Food & Water Preparation", slug: "food-water-preparation", description: "Water filtration and everyday food preparation appliances." },
  { name: "Cookware & Kitchen", slug: "cookware-kitchen", description: "Cookware, pressure cookers, kettles and gas cookers." },
  { name: "Cooling & Air Care", slug: "cooling-air-care", description: "Air conditioners and fans for homes and businesses." },
  { name: "Home Electronics", slug: "home-electronics", description: "Televisions, clocks and practical household electronics." },
  { name: "Refrigeration", slug: "refrigeration", description: "Full-size and compact Camy refrigeration products." },
];
...
function syncCompanies() {
  ...
  companies.forEach((company, index) => {
    ...
    const result = insert.run(
      company.id, company.name, company.shortName, company.description,
      company.fullDescription, company.tagline, company.industry,
      company.established || null, company.website || null, company.location || null,
      JSON.stringify(company.features), JSON.stringify(company.awards || []), logoUrl,
      company.phone || null, company.email || null, index + 1,
    );
...
function syncProducts() {
  const categoryIds = new Map(db.prepare("SELECT id, slug FROM categories").all().map((row) => [row.slug, row.id]));
  const insert = db.prepare(`
  ...
  camyProducts.forEach((product) => {
    ...
    const sourcePath = asset ? join(projectRoot, "src", "assets", "products", asset) : join(projectRoot, "src", "assets", "hero-manufacturing.jpg");
    ...
    inserted += insert.run(product.name, slugify(product.name), shortDescription, longDescription, imageUrl, categoryId).changes;
  });
  return inserted;
}

function syncBrands() {
  const existingNames = new Set(db.prepare("SELECT name FROM brands").all().map((brand) => brand.name));
  const insert = db.prepare(`
