...
router.get('/', (req, res) => {
  try {
    ...
    const rows = category
      ? db
          .prepare(
            `${baseSql} AND category = ? ORDER BY datetime(COALESCE(publishedDate, createdAt)) DESC, id DESC`,
          )
          .all(category)
      : db
          .prepare(`${baseSql} ORDER BY datetime(COALESCE(publishedDate, createdAt)) DESC, id DESC`)
          .all();
    res.json(rows);
  } catch (error) {
...
router.get('/all', authenticateToken, isAdmin, (_req, res) => {
  try {
    const rows = db
      .prepare(
        'SELECT * FROM news ORDER BY datetime(COALESCE(publishedDate, createdAt)) DESC, id DESC',
      )
      .all();
    res.json(rows);
  } catch (error) {
  ...
  try {
    ...
    `,
      ...
      .run(
        ...
        summary.trim(),
        content.trim(),
        nullIfEmpty(category) || 'Corporate',
        featuredImage,
        nullIfEmpty(author),
        finalPublishedDate,
        finalStatus,
        nullIfEmpty(seoTitle),
        nullIfEmpty(seoDescription),
      );
...
router.put('/:id', authenticateToken, isAdmin, upload.single('featuredImage'), (req, res) => {
  try {
    ...
    `,
      ...
      summary,
      content,
      nullIfEmpty(req.body.category) || existing.category || 'Corporate',
      featuredImage,
      nullIfEmpty(req.body.author),
      finalPublishedDate,
      finalStatus,
      nullIfEmpty(req.body.seoTitle),
      nullIfEmpty(req.body.seoDescription),
      req.params.id,
    );
