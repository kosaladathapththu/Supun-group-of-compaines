...
const reasons = [
  { title: "Learn through real work", reveal: "Build practical, career-ready experience.", icon: GraduationCap, image: learningImage },
  { title: "Grow across a Group", reveal: "Connect your potential to eleven businesses.", icon: Users, image: growthImage },
  { title: "Make useful change", reveal: "Turn good ideas into visible improvements.", icon: Sparkles, image: changeImage },
];
        ...
        <div className="career-shell career-hero-grid relative z-10">
          <div className="pb-16 pr-6 md:pb-24 lg:pr-16">
            ...
            <div className="mt-9 flex flex-wrap gap-3">
              <a href="#opportunities" className="inline-flex items-center gap-3 rounded-full bg-[#dca43a] px-7 py-4 text-sm font-bold text-[#10233f] transition hover:bg-[#78be43]">
                View opportunities <ArrowRight size={17} />
              </a>
              <a href="#talent" className="inline-flex items-center rounded-full border border-white/30 bg-white/5 px-7 py-4 text-sm font-bold backdrop-blur transition hover:bg-white/10">
                Send your profile
              </a>
            </div>
          </div>
        ...
        <div className="career-shell">
          ...
          <div className="area-grid mt-12">
            {areas.map(({ name, text, icon: Icon }) => (
              <a key={name} href="#opportunities" onClick={() => setSelected(name)} className="area-card group">
                <span className="grid h-14 w-14 place-items-center rounded-2xl bg-[#f8f0df] text-[#9a6b16]"><Icon /></span>
                <span className="mt-auto">
                  <small className="font-bold uppercase tracking-[.15em] text-[#315f9f]">Career area</small>
                  <h3 className="mt-3 text-2xl font-bold">{name}</h3>
                  <p className="mt-3 text-sm leading-6 text-[#56677c]">{text}</p>
                  <span className="mt-6 inline-flex items-center gap-2 text-sm font-bold">Explore <ArrowRight size={15} /></span>
                </span>
              </a>
            ))}
          </div>
            ...
            {reasons.map(({ title, reveal, icon: Icon, image }) => (
              <article key={title} className="why-card" tabIndex={0}>
                ...
                <div className="why-content">
                  <span className="why-icon"><Icon size={21} /></span>
