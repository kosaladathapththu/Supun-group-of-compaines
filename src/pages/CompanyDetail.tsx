...
import { ArrowLeft, ArrowRight, BriefcaseBusiness, CalendarDays, Check, ExternalLink, Globe2, Mail, MapPin, Phone } from "lucide-react";
...
const CompanyDetail = () => {
  ...
  return <div className="min-h-screen bg-[#f3f6f8] text-[#102746]">
    ...
    <section className="relative isolate min-h-[520px] overflow-hidden pb-14 pt-32 text-white md:flex md:items-end md:pb-16 md:pt-40">
      ...
      <div className="container mx-auto px-4"><div className="mb-7 flex flex-wrap items-center gap-4"><Link to="/companies" className="inline-flex items-center gap-2 text-sm font-semibold text-white/75 transition hover:text-[#efbd55]"><ArrowLeft size={16} />All Group Companies</Link>{company.website && <a href={company.website} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 rounded-full border border-white/25 bg-white/10 px-4 py-2 text-sm font-semibold text-white backdrop-blur transition hover:border-[#efbd55] hover:bg-[#efbd55] hover:text-[#071d37]">Official website <ExternalLink size={15} /></a>}</div><div className="company-detail-hero-grid grid items-end gap-7">
        ...
        {company.website ? <a href={company.website} target="_blank" rel="noopener noreferrer" aria-label={`Visit ${company.shortName} official website`} className="group justify-self-start rounded-xl focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#efbd55] focus-visible:ring-offset-4 focus-visible:ring-offset-[#071d37]"><CompanyLogo companyId={company.id} companyName={company.shortName} className="company-detail-hero-logo h-28 w-full max-w-[250px] rounded-xl border border-white/40 p-4 shadow-2xl transition duration-300 group-hover:-translate-y-1 group-hover:border-[#efbd55]" imageClassName="h-full w-full object-contain" /></a> : <CompanyLogo companyId={company.id} companyName={company.shortName} className="company-detail-hero-logo h-28 w-full max-w-[250px] justify-self-start rounded-xl border border-white/40 p-4 shadow-2xl" imageClassName="h-full w-full object-contain" />}
      </div></div>
    </section>
    ...
    <section className="py-12 md:py-16"><div className="company-detail-content-grid container mx-auto grid max-w-7xl gap-8 px-4">
      <div className="company-detail-primary min-w-0 space-y-8">
        ...
        {company.awards && company.awards.length > 0 && <article className="overflow-hidden rounded-2xl border border-[#102746]/10 bg-white shadow-[0_10px_30px_rgba(16,39,70,.06)]"><div className="px-6 pb-4 pt-5 sm:px-7 sm:pt-6"><p className="text-[10px] font-bold uppercase tracking-[.16em] text-[#315f9f]">Awards &amp; recognition</p><h2 className="mt-1.5 text-xl font-semibold normal-case">Recognised for excellence</h2></div><div className={`grid gap-px bg-[#102746]/10 ${company.awards.length > 1 ? "md:grid-cols-2" : ""}`}>{company.awards.map((award) => { const [awardName, issuer = "Industry recognition"] = award.split(" — "); const isCertification = awardName.toLowerCase().includes("made in sri lanka"); return <div key={award} className={`grid min-h-[165px] items-center gap-5 p-5 sm:grid-cols-[8.5rem_1fr] sm:px-7 ${isCertification ? "bg-[#fffaf0]" : "bg-white"}`}><div className="flex h-24 items-center justify-center rounded-xl border border-[#102746]/8 bg-white px-3 shadow-sm"><AwardIssuerMark issuer={issuer} certification={isCertification} /></div><div><span className="text-[8px] font-bold uppercase tracking-[.18em] text-[#102746]/35">{isCertification ? "National certification" : "Industry recognition"}</span><h3 className="mt-2 text-lg font-semibold normal-case leading-6 text-[#102746]">{awardName}</h3><p className="mt-2.5 text-sm text-[#728093]">Awarded by {issuer}</p></div></div>; })}</div></article>}
      </div>
