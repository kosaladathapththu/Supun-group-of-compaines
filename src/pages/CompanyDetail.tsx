  ...
  return <div className="min-h-screen bg-[#f3f6f8] text-[#102746]">
    ...
    <section className="py-12 md:py-16"><div className="company-detail-content-grid container mx-auto grid max-w-7xl gap-8 px-4">
      <div className="company-detail-primary min-w-0 space-y-8">
        ...
        {company.awards && company.awards.length > 0 && <article className="overflow-hidden rounded-2xl border border-[#102746]/10 bg-white shadow-[0_10px_30px_rgba(16,39,70,.06)]"><div className="p-7 pb-5 sm:px-9 sm:pt-9"><p className="text-xs font-bold uppercase tracking-[.16em] text-[#315f9f]">Awards &amp; recognition</p><h2 className="mt-2 text-2xl font-semibold normal-case">Recognised for excellence</h2></div><div className={`grid gap-px bg-[#102746]/10 ${company.awards.length > 1 ? "md:grid-cols-2" : ""}`}>{company.awards.map((award) => { const [awardName, issuer = "Industry recognition"] = award.split(" — "); const isCertification = awardName.toLowerCase().includes("made in sri lanka"); return <div key={award} className={`grid min-h-[220px] items-center gap-7 p-7 sm:grid-cols-[10rem_1fr] sm:px-9 ${isCertification ? "bg-[#fffaf0]" : "bg-white"}`}><div className="flex h-28 items-center justify-center rounded-2xl border border-[#102746]/8 bg-white px-4 shadow-sm"><AwardIssuerMark issuer={issuer} certification={isCertification} /></div><div><span className="text-[9px] font-bold uppercase tracking-[.18em] text-[#102746]/35">{isCertification ? "National certification" : "Industry recognition"}</span><h3 className="mt-3 text-xl font-semibold normal-case leading-7 text-[#102746]">{awardName}</h3><p className="mt-4 text-sm text-[#728093]">Awarded by {issuer}</p></div></div>; })}</div></article>}
      </div>
