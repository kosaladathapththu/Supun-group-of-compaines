...
import { Search, ShoppingBag, SlidersHorizontal, ListFilter, Sparkles } from 'lucide-react';
...
function getProductColorOptions(
  product: Product,
): Array<{ label: string; cssValue: string | null }> {
  if (!product.variations || product.variations.length === 0) {
    return [];
  }
          ...
          <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
            <div>
              ...
              <p className="text-muted-foreground max-w-2xl">
                Explore products with Shopify-style filtering. Use the left panel to search and
                refine by category.
              </p>
            </div>
            <div className="text-sm text-muted-foreground">
              Showing <span className="font-semibold text-primary">{filteredProducts.length}</span>{' '}
              of <span className="font-semibold text-primary">{products.length}</span> products
            </div>
          </div>
        </section>
          ...
          <aside className="rounded-2xl border border-primary/10 bg-white/95 p-4 md:p-5 shadow-sm h-fit lg:sticky lg:top-24">
            ...
            <div className="space-y-6">
              <div className="space-y-2">
                <label htmlFor="shop-search" className="text-sm font-medium text-primary">
                  Search
                </label>
            ...
            {isLoading && (
              <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
                {Array.from({ length: PRODUCTS_PER_PAGE }).map((_, index) => (
                  <div
                    key={index}
                    className="h-80 rounded-2xl bg-white border border-border animate-pulse"
                  />
                ))}
              </div>
            )}
                    ...
                    return (
                      ...
                      >
                        ...
                        <CardContent className="p-5">
                          <Link to={`/shop/${product.id}`}>
                            <h3 className="text-xl text-primary line-clamp-1 hover:underline">
                              {product.title}
                            </h3>
                          </Link>
                                      ...
                                      {colorOptions.slice(0, 5).map((colorOption) => (
                                        ...
                                        >
                                          <span
                                            ...
                                            style={{
                                              backgroundColor: colorOption.cssValue || '#d4d4d8',
                                            }}
                                          />
                                        </span>
                                      ))}
