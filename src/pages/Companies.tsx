      ...
      <div className="space-y-5">{visibleCompanies.map((company, index) => {
        ...
        return <article key={company.id} className="group overflow-hidden rounded-2xl border border-[#0b2747]/10 bg-white shadow-[0_10px_32px_rgba(11,39,71,.07)] transition duration-300 hover:-translate-y-0.5 hover:shadow-[0_18px_45px_rgba(11,39,71,.12)]"><div className="grid lg:grid-cols-3">
          ...
          <div className={`flex flex-col p-6 sm:p-8 lg:col-span-2 lg:row-start-1 ${imageOnRight ? "lg:col-start-1" : "lg:col-start-2"}`}>
            ...
            <div className="grid flex-1 gap-6 py-5 md:grid-cols-[1.25fr_.75fr]"><p className="text-sm font-medium leading-6 text-[#34465b]">{company.fullDescription}</p><div><p className="mb-3 text-xs font-bold text-[#244f83]">Key Features</p><ul className="space-y-2">{company.features.slice(0, 4).map((feature) => <li key={feature} className="flex gap-2 text-xs font-medium leading-5 text-[#41546a]"><Check size={15} className="mt-0.5 shrink-0 text-[#d79a22]" strokeWidth={2.5} />{feature}</li>)}</ul></div></div>
