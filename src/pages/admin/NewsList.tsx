...
export default function NewsList() {
  ...
  const filtered = useMemo(() => {
    ...
    return articles.filter((article) =>
      `${article.title} ${article.category} ${article.summary}`.toLowerCase().includes(q),
    );
  }, [articles, searchTerm]);
    ...
    <div className="container mx-auto px-4 py-8">
      <Card>
        <CardHeader>
          <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            <CardTitle className="flex items-center gap-2 text-2xl">
              <Newspaper className="h-6 w-6" /> News & Media
            </CardTitle>
        ...
        <CardContent>
          ...
          {isLoading ? (
            <div className="py-10 text-center text-muted-foreground">Loading news...</div>
          ) : filtered.length === 0 ? (
            <div className="py-12 text-center">
              ...
              <p className="mt-2 text-muted-foreground">
                Create the first approved update from the admin panel.
              </p>
            </div>
          ) : (
      ...
      <AlertDialog open={deleteId !== null} onOpenChange={(open) => !open && setDeleteId(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This permanently removes the article from the admin panel and public website.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
