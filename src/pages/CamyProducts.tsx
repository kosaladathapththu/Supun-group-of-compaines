...
const categoryLinks: Record<string, string> = {
  ...
  'Electric Kettles':
    'https://www.anythingatsupun.lk/product-category/electronics/electric-kettles/',
        ...
        <div className="camy-hero container mx-auto px-6 lg:px-12">
          <div className="max-w-2xl">
            <img
              src={camyRedLogo}
              alt="Camy"
              className="mb-7 h-32 w-64 object-contain object-left md:h-40 md:w-80"
            />
            <p className="text-xs font-bold uppercase tracking-[.22em] text-[#ed1c24]">
              A Supun Group brand
            </p>
            <h1
              className="mt-5 max-w-xl font-bold uppercase"
              style={{
                fontSize: 'clamp(3rem,5.2vw,5.1rem)',
                lineHeight: '.94',
                letterSpacing: '-.045em',
              }}
            >
            ...
            <p className="mt-7 max-w-xl text-lg leading-8 text-black/60 md:text-xl">
              Camy brings together a growing range of products for the home, the road and everything
              in between. Designed with purpose and shaped by an understanding of everyday needs,
              each product is backed by the manufacturing experience of the Supun Group.
            </p>
            <div className="mt-8 grid gap-4 sm:grid-cols-2">
              <article className="border-l-2 border-[#ed1c24] pl-4">
                <h2 className="text-xs font-extrabold uppercase tracking-[.16em] text-[#111]">
                  Designed with purpose
                </h2>
                <p className="mt-2 text-sm leading-6 text-black/55">
                  We focus on the details that make a product useful, how it fits into your routine,
                  how it performs and how it feels to live with.
                </p>
              </article>
              <article className="border-l-2 border-[#ed1c24] pl-4">
                <h2 className="text-xs font-extrabold uppercase tracking-[.16em] text-[#111]">
                ...
                <p className="mt-2 text-sm leading-6 text-black/55">
                  CAMY is backed by the manufacturing experience of the Supun Group, bringing local
                  knowledge and hands-on expertise to everything we make.
                </p>
              </article>
            </div>
            <div className="camy-actions">
              <a href="#camy-range" className="camy-button camy-button-dark">
                Explore the range <ArrowRight size={18} />
              </a>
              <a
                href={shopUrl}
                className="camy-button camy-button-red"
                target="_blank"
                rel="noopener noreferrer"
              >
                <ExternalLink size={17} /> Shop online
              </a>
            </div>
          </div>
          <div className="camy-visual">
            ...
            <div className="camy-proof">
              ...
              <div className="camy-proof-item">
                ...
                <p className="camy-proof-text">
                  Built on the Group's hands-on manufacturing expertise.
                </p>
              </div>
            </div>
          </div>
      ...
      <section id="camy-range" className="py-20 md:py-28">
        <div className="container mx-auto px-6 lg:px-12">
          <div className="mb-12 flex flex-col justify-between gap-5 border-b border-black/15 pb-8 md:flex-row md:items-end">
            <div>
              <p className="text-xs font-bold uppercase tracking-[.24em] text-[#ed1c24]">
                The Camy range
              </p>
              <h2 className="mt-4 max-w-3xl text-4xl font-bold uppercase tracking-[-.035em] md:text-6xl">
                Everything you need, in one place.
              </h2>
            </div>
            <p className="max-w-md text-base leading-7 text-black/55">
              From the road to the kitchen and from cooling to home entertainment, explore the CAMY
              range.
            </p>
          </div>
            ...
            {displayProducts.map((product) => (
              <article key={product.id} className="camy-card">
                ...
                <div className="camy-copy">
                  <div>
                    <span className="camy-tag">
                      <ShieldCheck size={15} />
                      {product.note}
                    </span>
                  </div>
                  <div className="py-7">
                    <h3 className="text-2xl font-bold tracking-[-.025em] md:text-3xl">
                      {product.name}
                    </h3>
                    <p className="mt-3 text-sm leading-6 text-black/50">
                      Manufactured in Sri Lanka by{' '}
                      <strong className="font-semibold text-black/75">{product.madeBy}</strong>.
                    </p>
                  </div>
                  <a
                    href={categoryLinks[product.name] || shopUrl}
      ...
      <section className="pb-20 md:pb-28">
        <div className="container mx-auto px-6 lg:px-12">
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="flex flex-col items-start justify-between gap-8 rounded-[2rem] bg-[#111] p-8 text-white md:flex-row md:items-center md:p-12"
          >
            <div className="flex items-center gap-6">
              <span className="rounded-2xl bg-white p-4">
                <img
                  src={anythingAtSupunLogo}
                  alt="Anything at Supun"
                  className="h-14 w-auto object-contain"
                />
              </span>
              <div>
                <p className="text-xs font-bold uppercase tracking-[.2em] text-[#ed1c24]">
                  Official online store
                </p>
                <h2 className="mt-2 text-2xl font-bold md:text-4xl">
                  Find Camy at Anything at Supun.
                </h2>
              </div>
            </div>
            <span className="camy-button camy-button-red shrink-0">
              Shop collection <ExternalLink size={17} />
            </span>
          </a>
        </div>
      </section>
