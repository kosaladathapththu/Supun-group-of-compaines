  ...
  <div className="min-h-screen overflow-hidden bg-[#f7f7f4] text-[#10233f]">
    ...
    <section className="bg-[#f1f0eb] py-20 md:py-28">
      <div className="container mx-auto px-4">
        ...
        <div className="grid gap-px overflow-hidden rounded-[2rem] border border-[#10233f]/10 bg-[#10233f]/10 md:grid-cols-2">{leadership.map((person, index) => <AnimatedSection key={person.name} animation="slide-up" delay={(index % 2) * 80} duration={750} triggerOnce={false}><article className="group relative h-full bg-[#faf9f5] p-7 transition duration-300 hover:bg-[#f1f5ed] sm:p-9"><div className="flex items-start gap-5"><span className="mt-1 text-xs font-semibold tracking-[.18em] text-[#78be43]">{String(index + 1).padStart(2, "0")}</span><div><h3 className="text-2xl font-semibold normal-case text-[#10233f]">{person.name}</h3><p className="mt-2 text-sm font-semibold uppercase tracking-[.08em] text-[#536275]">{person.title}</p>{person.description && <p className="mt-5 max-w-xl leading-relaxed text-[#687269]">{person.description}</p>}</div></div><span className="absolute bottom-0 left-0 h-0.5 w-0 bg-[#78be43] transition-all duration-500 group-hover:w-full" /></article></AnimatedSection>)}</div>
      </div>
    </section>
