...
export default function News() {
  ...
  useEffect(() => {
    (async () => {
      try {
        setArticles(await newsAPI.getPublished());
      } catch (error) {
    ...
    <main className="min-h-screen bg-[#f5f6f4] text-[#10233f]">
      ...
      <section className="px-5 py-16 md:px-8 md:py-24">
        <div className="mx-auto max-w-7xl">
          ...
          {isLoading ? <div className="py-20 text-center text-[#68788d]">Loading latest updates...</div> : loadError ? (
            <div className="mt-8 rounded-3xl border border-[#10233f]/10 bg-white p-12 text-center"><Newspaper className="mx-auto text-[#78be43]" size={34} /><h2 className="mt-5 text-2xl font-bold">News service is not available in this preview.</h2><p className="mx-auto mt-3 max-w-2xl text-[#68788d]">The page is ready for the Namecheap backend. Once the API is running, published admin articles will appear here automatically.</p></div>
          ) : visible.length === 0 ? (
            ...
            <div className="mt-8 space-y-6">
              {featured && <Link to={`/news/${featured.slug}`} className="group grid overflow-hidden rounded-[2rem] border border-[#10233f]/10 bg-white shadow-sm lg:grid-cols-[1.15fr_.85fr]">
                <div className="min-h-[300px] bg-[#dfe6ed] lg:min-h-[430px]">{featured.featuredImage ? <img src={getFileUrl(featured.featuredImage) || ''} alt={featured.title} className="h-full w-full object-cover transition duration-700 group-hover:scale-[1.02]" /> : <div className="flex h-full items-center justify-center"><Newspaper size={52} className="text-[#315f9f]/35" /></div>}</div>
