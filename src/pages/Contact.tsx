...
import { ArrowRight, ArrowUpRight, Clock3, Mail, MapPin, Phone, Send, ShieldCheck } from "lucide-react";
  ...
  return (
    <main className="contact-page min-h-screen text-[#10233f]">
      ...
      <style>{`
        ...
        .directory-grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:18px}.company-card{--brand:#315f9f;position:relative;display:flex;min-height:330px;flex-direction:column;overflow:hidden;border:1px solid rgba(16,35,63,.11);border-radius:24px;background:#fff;box-shadow:0 8px 28px rgba(16,35,63,.045);transition:transform .3s ease,border-color .3s ease,box-shadow .3s ease}
        .company-card::before{content:"";position:absolute;z-index:2;inset:0 auto auto 0;width:100%;height:4px;background:var(--brand)}.company-card:hover{transform:translateY(-6px);border-color:color-mix(in srgb,var(--brand) 38%,transparent);box-shadow:0 22px 55px rgba(16,35,63,.12)}
        .company-logo-zone{position:relative;display:flex;height:112px;align-items:center;justify-content:center;overflow:hidden;border-bottom:1px solid rgba(16,35,63,.08);background:color-mix(in srgb,var(--brand) 7%,white)}.company-logo-zone::after{content:"";position:absolute;width:160px;height:160px;border:28px solid color-mix(in srgb,var(--brand) 8%,transparent);border-radius:50%;right:-70px;top:-88px}.company-logo{position:relative;z-index:1;width:190px;height:76px;background:transparent!important}.company-card-body{display:flex;flex:1;flex-direction:column;padding:22px 24px 20px}.company-sector{color:var(--brand);background:color-mix(in srgb,var(--brand) 8%,white);border-color:color-mix(in srgb,var(--brand) 18%,white)}
        .company-contact-row{display:flex;min-height:43px;align-items:center;gap:10px;border-top:1px solid rgba(16,35,63,.08);padding-top:14px;color:#53657b}.company-contact-row svg{color:var(--brand)}.company-action{display:flex!important;width:auto!important;height:42px!important;align-items:center!important;justify-content:center!important;gap:8px;padding:0 17px!important;border-radius:999px;background:var(--brand);color:#fff;font-size:12px;font-weight:800;white-space:nowrap;transition:.25s ease}.company-action svg{position:static!important;inset:auto!important;display:block!important;margin:0!important;transform:none!important}.company-card:hover .company-action{filter:brightness(.9);transform:translateX(2px)}
        ...
        <div className="contact-shell"><div className="grid gap-7 border-b border-[#10233f]/10 pb-9 lg:grid-cols-[1fr_.65fr] lg:items-end"><div><p className="text-xs font-bold uppercase tracking-[.23em] text-[#5c9c30]">Company directory</p><h2 className="mt-4 text-4xl font-bold tracking-[-.045em] md:text-5xl">Contact a business directly.</h2></div><p className="max-w-lg text-sm leading-7 text-[#6d7c90] lg:justify-self-end">Already know who you need? Connect directly with a Supun Group company.</p></div>
          <div className="directory-grid mt-8">{companies.map((company) => {
            ...
            return <article key={company.id} className="company-card group" style={{"--brand":brandColours[company.id] || "#315f9f"} as CSSProperties}>
              <div className="company-logo-zone"><CompanyLogo companyId={company.id} companyName={company.shortName} className="company-logo" imageClassName="max-h-[68px] max-w-[175px]" /></div>
